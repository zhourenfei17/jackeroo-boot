...
import cn.hub.jackeroo.utils.JaxbMapper;
...
public class GenUtils {
	...
	/*public static void initColumnField(GenTable genTable) {
		for (GenTableColumn column : genTable.getColumnList()) {
			...
			*//*
			 ...
			 * column.setJavaField(column.getJavaField().replaceAll("Id", ".id|name"));
			 * column.setShowType("officeselect"); } // 区域 else if
			 * (StringUtils.startsWithIgnoreCase(column.getName(), "area_id")){
		...
		try {
			...
			if (file != null) {
				return file.getAbsolutePath() + File.separator
				        + StringUtils.replaceEach(GenUtils.class.getName(), new String[] { "util." + GenUtils.class.getSimpleName(), "." },
	...
	*//**
	...
	public static <T> T fileToObject(String fileName, Class<?> clazz) {
		...
		// String pathName = StringUtils.replace(getTemplatePath() + "/" + fileName,
		// "/", File.separator);
		// log.debug("file to object: {}", pathName);
		// String content = "";
		// try {
	...
	*//**
	...
	public static Map<String, Object> getDataModel(GenScheme genScheme) {
		...
		model.put("functionAuthor",
		          StringUtils.isNotBlank(genScheme.getFunctionAuthor()) ? genScheme.getFunctionAuthor() : UserUtils.getUser().getName());
		...
		model.put("dbType", Global.jdbcUrl);
