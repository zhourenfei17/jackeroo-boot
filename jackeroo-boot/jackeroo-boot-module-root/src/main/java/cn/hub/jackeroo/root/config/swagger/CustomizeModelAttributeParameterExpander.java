...
public class CustomizeModelAttributeParameterExpander extends ModelAttributeParameterExpander {
    ...
    public static void main(String[] args) {
        Set<Field> fieldSet = Reflections.getAccessibleFields(SysRole.class);
    ...
    private Set<PropertyDescriptor> propertyDescriptors(final Class<?> clazz) {
        try {
            ...
            Map<String, Field> filedsMap = Reflections.getAccessibleFields(clazz).stream().collect(Collectors.toMap(Field::getName, self -> self, (v1, v2) -> v1));

            for (PropertyDescriptor descriptor : descriptors) {
                Field field = filedsMap.get(descriptor.getName());
                /*try {
                    field = clazz.getDeclaredField(descriptor.getName());
                } catch (Exception e) {
                    log.debug(String.format("Failed to get bean properties on (%s) field (%s)", clazz, descriptor.getName()), e);
                }*/

                if (field != null) {
                    field.setAccessible(true);
                    ApiParam apiParam = field.getDeclaredAnnotation(ApiParam.class);
